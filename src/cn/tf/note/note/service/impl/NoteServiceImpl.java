package cn.tf.note.note.service.impl;
...
import cn.tf.note.note.dao.CreateIndexDao;
import cn.tf.note.note.dao.DataDao;
import cn.tf.note.note.dao.RedisDao;
import cn.tf.note.note.dao.SearchIndexDao;
import cn.tf.note.note.service.NoteService;
...
public class NoteServiceImpl implements NoteService {
	...
	public List<NoteBook> getAllNoteBook(String userIdAndName)
		...
		if (notebookNames == null || notebookNames.size() <= 0) {// 如果从redis中查不到值，去hbase中查询
			notebookNames=new ArrayList<NoteBook>();
			ResultScanner queryByReg = dataDao.queryByReg(
					Constants.NOTEBOOK_TABLE_NAME, userIdAndName + "*");
			for (Result row : queryByReg) {// 循环结果集
				NoteBook noteBook = new NoteBook();
